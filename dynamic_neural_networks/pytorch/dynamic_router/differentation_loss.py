...
LR_R = 5e-4
LR_A = 5e-5
NAME = "differentation-loss-sdigan-intensity-aux-reg-1-disc-3-experts"
...
for _ in range(N_RUNS):
    ...
    def train_step(batch):
        ...
        mean_intensities_experts = torch.zeros(3, requires_grad=True).to(device)  # mean intensities for each expert for each batch
        ...
        # Assume that mean_intensities_experts is computed from non-leaf tensors
        # After computing the mean intensities, retain gradients
        mean_intensities_experts.retain_grad()
        ...
        router_loss = gen_loss_scaled + differentiation_loss
        ...
        return gen_losses[0].item(), gen_losses[1].item(), gen_losses[2].item(),\
               ...
               std_intensities_experts.detach().cpu().numpy(), mean_intensities_experts.detach().cpu().numpy(),\
               differentiation_loss.cpu().item()
    ...
    def train(train_loader, epochs, y_test):
        ...
        for epoch in range(epochs):
            ...
            for batch in train_loader:
                ...
                mean_intensities_experts, differentation_loss = train_step(batch)
