...
def train_step(batch):
    ...
    differentation_loss = - (mu[0] - mu[1]) ** 2 - (mu[0] - mu[2]) ** 2 - (mu[1] - mu[2]) ** 2
    router_loss = gen_losses.mean().clone() * GEN_STRENGTH +\
                  differentation_loss * DIFF_STRENGTH +\
                  expert_utilization_entropy * ENT_STRENGTH
...
def train(train_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            aux_reg_loss, n_chosen_experts_batch, differentation_loss = train_step(batch)
