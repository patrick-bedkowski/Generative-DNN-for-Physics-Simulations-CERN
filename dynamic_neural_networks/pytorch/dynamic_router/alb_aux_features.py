...
for _ in range(N_RUNS):
    ...
    data = pd.read_pickle(DATA_IMAGES_PATH)#[:20000]
    ...
    def generator_train_step(generator, discriminator, a_reg, cond, g_optimizer, a_optimizer, criterion,
        ...
        # validate two images
        fake_output, fake_latent = discriminator(fake_images.clone().detach(), cond)  # is it correct?
        fake_output_2, fake_latent_2 = discriminator(fake_images_2.clone().detach(), cond)
    ...
    def discriminator_train_step(disc, generator, d_optimizer, criterion, class_counts, real_images, cond, batch_size) -> np.float32:
    ...
    def train_step(batch, epoch):
        ...
        for i in range(N_EXPERTS):
            ...
            selected_discriminator = discriminators[i]
            selected_aux_reg = aux_regs[0]
            selected_aux_reg_optimizer = aux_reg_optimizers[0]
            selected_class_counts = class_counts_adjusted[i]

            gen_loss, div_loss, intensity_loss, \
            aux_reg_loss, std_intensity, mean_intensity, mean_intensities, aux_reg_features = generator_train_step(selected_generator,
        ...
        if N_EXPERTS > 1:
            ...
            def compute_differentiation_loss(aux_reg_features_experts: List):
                """
    ...
    config_wandb = {
        "disc_type_features": disc_type_features,
        "Model": NAME,
        "dataset": "proton_data",
        "n_experts": N_EXPERTS,
        "epochs": EPOCHS,
    ...
    run.log_code(name="alb_aux_features.py")
