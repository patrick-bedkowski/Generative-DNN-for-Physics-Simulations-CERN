...
from models_pytorch import GeneratorNeutron, DiscriminatorNeutron, RouterNetwork, AuxRegNeutron
...
NAME = f"neutron-test-diff_detach_sep_aux_3_exp_clip_{CLIP_DIFF_LOSS}_diff_expert_distribution_utilization_{ED_STRENGTH}_{GEN_STRENGTH}_{UTIL_STRENGTH}_no_stop_{STOP_ROUTER_TRAINING_EPOCH}"

for _ in range(N_RUNS):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # device = torch.device("cuda:0")
    data = pd.read_pickle('/net/tscratch/people/plgpbedkowski/data/neutron/data_neutron_photonsum_neutron_1_3360.pkl')
    data_cond = pd.read_pickle('/net/tscratch/people/plgpbedkowski/data/neutron/data_cond_neutron_photonsum_neutron_1_3360.pkl')
    ...
    data_posi = pd.read_pickle('/net/tscratch/people/plgpbedkowski/data/neutron/data_coord_photonsum_neutron_1_3360.pkl')
    ...
    data_cond = scaler_cond.fit_transform(data_cond.drop(columns=["std", "neutron_photon_sum",
                                                                  'group_number'])).astype(np.float32)
    ...
    if SAVE_EXPERIMENT_DATA:
        ...
        save_scales("Neutron", scaler_cond.mean_, scaler_cond.scale_, filepath)
    ...
    def discriminator_train_step(disc, generator, d_optimizer, criterion, real_images, cond, BATCH_SIZE):
        ...
        print('REAL IMAGE SHAPE NEUTRON', real_images.shape)
    ...
    def train(train_loader, epochs, y_test):
        ...
        for epoch in range(0, epochs):
            ...
            if len(indices_expert_0) > 0:
                ...
                ch_org_0 = np.array(org_0).reshape(-1, 44, 44)
                del org_0
