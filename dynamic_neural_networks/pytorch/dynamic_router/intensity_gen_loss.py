...
LR_R = 1e-3
LR_A = 5e-5
NAME = "router-generator-loss-sdigan-intensity-aux-reg-1-disc-3-experts"
...
def train_step(batch):
    ...
    router_loss = aux_reg_loss.clone().detach()
    ...
    return gen_losses[0], gen_losses[1], gen_losses[
        2], total_disc_loss.item(), router_loss.item(), div_loss.cpu().item(),\
...
def train(train_loader, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            aux_reg_loss, n_chosen_experts_batch = train_step(batch)
            ...
            n_chosen_experts.append(n_chosen_experts_batch)
        ...
        # calculate average number of choosen expert in each batch
        n_chosen_experts_mean_epoch = np.mean(np.array(n_chosen_experts), axis=0)
        ...
        log_data = {
            ...
            'n_choosen_experts_mean_epoch_0': n_chosen_experts_mean_epoch[0],
            'n_choosen_experts_mean_epoch_1': n_chosen_experts_mean_epoch[1],
            'n_choosen_experts_mean_epoch_2': n_chosen_experts_mean_epoch[2],
